...
class Tracker_from_det(Tracker_from_ref_annotation) :
    ...
    def extract_reid_feature(self, set_of_bdb_index, image):
        ...
        for i, index in enumerate(set_of_bdb_index):
            track = self._tracklets[index]
...
#-----------------------------------------------------------------------------------------------------
    def matching_reid(self, set_of_negative_index=[], set_of_abandonned_index=[]):
        ...
        if dists.shape[0] * dists.shape[1] > 0 :
            ...
            for i in range(len(dists_argmin)) :
                if dists_min[i] < 0.84 :
                    matches.append([set_of_abandonned_index[i], set_of_negative_index[dists_argmin[i]], dists_min[i]])
    ...
    def refine_matching_reid(self, matches) :
        ...
        accepted_tracks = []
        accepted_new_dets = []
        dists_between_tracks = {}
        for m in matches:
            # for each det, get the track with highest iou score
            if m[1] not in dists_between_tracks or m[2] < dists_between_tracks[m[1]][2]:
                dists_between_tracks[m[1]] = m
        for i in dists_between_tracks :
    ...
    def merge_tracks_reid(self, match, frame_id, image):
        ...
        for m in match:
            ...
            '''bdb_tracks.append(self._tracklets[m[0]]._bounding_box)
            # set feature Re_ID
        features = extract_reid_features(self.reid_model, image, bdb_tracks)  # dung CNN de extract cls_feature to
        features = features.cpu().numpy()
        ...
        self.remove_abandonned_tracks([t[1] for t in match])



    def ready_for_new_id(self, index_new_id=None):
        for index in self._tracklets :
